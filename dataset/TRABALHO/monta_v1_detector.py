...
Uso:
  python monta_v1_detector.py --vista lateral [--ensaio]
"""
...
ROI_JSON = DS / 'TRABALHO' / 'roi-por-camera.json'
sys.path.insert(0, str(DS / 'TRABALHO'))
try:
    from roi_por_camera import camera_do_caminho      # mesma regra usada para derivar a ROI
except Exception:                                     # pragma: no cover
    def camera_do_caminho(_rel):
        return 'desconhecida'
...
def chave_de_item(rel: str) -> str:
    ...
    if partes and partes[0] == 'dataset' and len(partes) >= 3:
        nome = p.stem
        m = CONTADOR.match(nome)
        if m and m.group('numero'):
            base = f"{partes[1]}/{'/'.join(partes[2:-1])}"
            return f'seq:{base}:{m.group("prefixo")}'    # 2) sequência/trig
...
def main() -> int:
    ...
    if not a.ignorar_frescor:
        ...
        _guarda = _sp.run([sys.executable, str(DS / 'TRABALHO' / 'guarda_frescor.py')],
        ...
        if _guarda.returncode != 0:
            print('ABORTADO pela trava de frescor: o Label Studio avancou desde o export '
                  '(rode dataset/TRABALHO/exporta_anotacoes.py e tente de novo)')
            return 4
    ...
    for i in itens:
        ...
        if caixa:
            ...
            with Image.open(src) as im:
                L, A = im.size
                x, y = int(caixa['x'] * L), int(caixa['y'] * A)
                w, h = int(caixa['w'] * L), int(caixa['h'] * A)
    ...
    (OUT / 'manifest.json').write_text(json.dumps({
        'vista': a.vista, 'classes': CLASSES, 'quando': datetime.now(timezone.utc).isoformat(timespec='seconds'),
