...
def main() -> int:
    ...
    sub.add_parser("ls")
    p_up = sub.add_parser("upload"); p_up.add_argument("local", type=Path)
    p_up.add_argument("destino", nargs="?")
    p_pull = sub.add_parser("pull"); p_pull.add_argument("remoto"); p_pull.add_argument("local", type=Path)
    p_rm = sub.add_parser("rm"); p_rm.add_argument("remoto")
    p_run = sub.add_parser("run"); p_run.add_argument("remoto"); p_run.add_argument("--segundos", type=int, default=60)
