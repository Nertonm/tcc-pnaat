...
def test_gate_rnf02_aprovado():
    prev = ["normal"] * 95 + ["cap_ausente"] * 2 + ["cap_mal_rosqueada"] * 3
    verd = ["normal"] * 95 + ["cap_ausente"] * 2 + ["cap_mal_rosqueada"] * 3
    assert avaliar(prev, verd, limiar=0.95)["aprova_rnf02"] is True


def test_confusao_reprova():
    res = avaliar(["normal"] * 90 + ["cap_ausente"] * 10, ["normal"] * 100, limiar=0.95)
    assert res["aprova_rnf02"] is False
    assert res["matriz_confusao"].get("normal->cap_ausente", 0) == 10
