from pocs.events import DefectClass, ViewResult
from pocs.poc04_fusao import fuse_views


def test_majoria_estrita_vence():
    res = fuse_views([ViewResult("v1", DefectClass.TAMPA_AUSENTE, 0.9), ViewResult("v2", DefectClass.TAMPA_AUSENTE, 0.85)])
    assert res[0] == DefectClass.TAMPA_AUSENTE
    assert res[1] == 0.9


def test_empate_vira_analise_humana():
    res = fuse_views([ViewResult("v1", DefectClass.TAMPA_AUSENTE, 0.8), ViewResult("v2", DefectClass.TAMPA_MAL_ROSQUEADA, 0.9)])
    assert res[0] == DefectClass.ANALISE_HUMANA


def test_sem_maioria_estrita_analise_humana():
    res = fuse_views([
        ViewResult("v1", DefectClass.TAMPA_AUSENTE, 0.9),
