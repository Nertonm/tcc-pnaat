...
def recorte_da_vista(imagem, roi: Roi):
    ...
    try:
        ...
        raise ErroDeOrquestracao(f"roi invalida (esperado x1,y1,x2,y2 finitos): {roi!r}") from None
    ...
    x1, y1, x2, y2 = (int(x1_rel * largura), int(y1_rel * altura),
                      int(x2_rel * largura), int(y2_rel * altura))
...
def _referencias_das_vistas(item: ItemCapturado) -> tuple[ReferenciaDaEvidencia, ...]:
    ...
    return tuple(ReferenciaDaEvidencia(vista=v.vista, caminho=str(v.imagem),
                                       sha256=hashlib.sha256(v.imagem.read_bytes()).hexdigest())
                 for v in item.vistas)


def executar(item: ItemCapturado, classificador: ClassificadorDeVista, registro: Registro,
             identidade: IdentidadeDoRig, roi: Roi | None = None,
    ...
    for vistacap in item.vistas_utilizaveis:
        ...
        for dominio in DOMINIOS_ESPERADOS:
            ...
            medidas.append(Medida(vista=decisao.vista, dominio=decisao.dominio, classe=decisao.classe,
                                  confianca=decisao.confianca, evidencias=decisao.evidencias))
    ...
    if check is not None and vistacap_do_check is not None:
        ...
        check_presente = True
    conformidade = avaliar_conformidade(medidas, config, check_escalonado=escalonado,
                                        check_motivo=motivo, check_presente=check_presente)
...
def criar_classificador(artefato: str | Path | None = None, *, fonte: str | None = None,
                        limiar: float | None = None, embutidor=None):
    ...
    """
    from classificador_artefato import ARTEFATO_PADRAO, FONTE_PADRAO, ClassificadorDoArtefato
    from classificador import ErroDeClassificacao
    from pathlib import Path as _Path

    if artefato is None:
        return SemModelo(), f"sem modelo de visao pedido (nenhum artefato: {ARTEFATO_PADRAO})"
    caminho = _Path(artefato)
    try:
        classificador = ClassificadorDoArtefato.abrir(
            caminho, fonte=fonte or FONTE_PADRAO, limiar_de_confianca=limiar, embutidor=embutidor)
...
def main(argv: list[str] | None = None) -> int:
    ...
    item = FonteDeDiretorio(a.captura, janela_s=a.janela).capturar(a.item, datetime.now(UTC))
    ...
    try:
        ...
        resultado = executar(item, classificador, registro,
                             IdentidadeDoRig(a.equipamento, a.localizacao), roi=tuple(a.roi))
        ...
        if item.fora_da_janela:
            print(f"  fora da janela: {[v.vista.value for v in item.fora_da_janela]} (janela={a.janela}s)")
        print(f"  itens no registro: {registro.contar()} | aprovados: {1 if resultado.aprovado else 0}")
    finally:
        registro.fechar()
    return 0
