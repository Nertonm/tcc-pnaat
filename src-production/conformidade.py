...
pode escalonar e NUNCA aprovar. Regras, todas testadas:
  ...
  2. defeito detectado em qualquer vista decisoria do dominio REPROVA, com precedencia declarada
     (no dominio da tampa: `tampa_ausente` > `tampa_mal_rosqueada`; empate de classe resolvido pela
     maior confianca);
...
PRECEDENCIA: dict[Dominio, tuple[Classe, ...]] = {
    Dominio.TAMPA: (Classe.TAMPA_AUSENTE, Classe.TAMPA_MAL_ROSQUEADA, Classe.NORMAL),
...
def avaliar_conformidade(medidas: Sequence[Medida], config: ConfiguracaoDoRig = ConfiguracaoDoRig(),
           check_escalonado: bool = False, check_motivo: str | None = None,
           check_presente: bool | None = None) -> ResultadoDaConformidade:
    """Aplica as regras.
    ...
    for m in medidas:
        if m.vista not in config.vistas_conhecidas:
            raise ErroDeConformidade(f"vista {m.vista.value!r} nao esta no rig declarado "
    ...
    for dominio in DOMINIOS_ESPERADOS:      # sempre os dois: dominio nao declarado tambem reporta
    ...
    discordancia = any("classes_divergentes" in m for r in resultados for m in r.motivos)
    ...
    if not _rig_completo(config):
        motivos.append(MOTIVO_RIG_INCOMPLETO)     # sempre visivel, nao so quando seria aprovado
        if status == "ok":
            status = "inconclusivo"
    return ResultadoDaConformidade(status=status, por_dominio=tuple(resultados),
                            discordancia_lateral=discordancia, motivos=tuple(motivos),
...
def _rig_completo(config: ConfiguracaoDoRig) -> bool:
    ...
    return (len(config.vistas_decisorias) >= VISTAS_DECISORIAS_ESPERADAS
            and config.check_obrigatorio
            and all(d in config.dominios_medidos for d in DOMINIOS_ESPERADOS))


def _avaliar_dominio(dominio: Dominio, medidas: Sequence[Medida],
                    config: ConfiguracaoDoRig) -> ResultadoDeDominio:
    ...
    if dominio not in config.dominios_medidos:
        return ResultadoDeDominio(dominio, "inconclusivo", None, origem,
    ...
    defeitos = [m for m in do_dominio if m.classe not in (Classe.NORMAL, Classe.INCONCLUSIVO)]
    if defeitos:
        classe = _por_precedencia(dominio, defeitos)
        return ResultadoDeDominio(dominio, "defeito", classe, origem, tuple(motivos))
    if (Classe.INCONCLUSIVO in classes or motivos
...
def _por_precedencia(dominio: Dominio, defeitos: Sequence[Medida]) -> Classe:
    ...
    raise ErroDeConformidade(f"classe de defeito fora da precedencia declarada para {dominio.value}")
