...
def construir_embutidor(config: Mapping[str, Any]) -> Callable[[np.ndarray], np.ndarray]:
    ...
    if tipo == "hub" and not config.get("repo"):
        raise ErroDeClassificacao(f"config_extrator tipo 'hub' sem 'repo': {dict(config)!r}")
    if tipo == "torchvision" and not config.get("fn"):
        raise ErroDeClassificacao(f"config_extrator tipo 'torchvision' sem 'fn': {dict(config)!r}")
    ...
    def carregar():  # pragma: no cover - exercitado no canario com torch real
        ...
        if tipo == "hub":
            modelo = torch.hub.load(config["repo"], config["fn"], pretrained=True, trust_repo=True)
        else:
            fabrica = getattr(torchvision.models, str(config["fn"]), None)
            if fabrica is None:
                raise ErroDeClassificacao(f"torchvision nao tem o modelo {config['fn']!r}")
            pesos = getattr(torchvision.models, f"{str(config['fn']).title().replace('_', '')}_Weights",
                             None)
            pesos = getattr(pesos, str(config["pesos"])) if pesos and config.get("pesos") else None
    ...
    def embutir(recorte_bgr: np.ndarray) -> np.ndarray:  # pragma: no cover - canario
        ...
        if estado is None:
            ...
            estado = (modelo, dispositivo, transforms.Compose([
                transforms.Resize((lado, lado)),
                transforms.ToTensor(),
                transforms.Normalize(mean=list(MEDIA_DA_IMAGENET), std=list(DESVIO_DA_IMAGENET)),
            ]))
        ...
        if isinstance(saida, dict):  # dinov2 devolve dict: o token CLS
            saida = saida.get("x_norm_clstoken", list(saida.values())[0])
...
class ClassificadorDoArtefato:
    ...
    def __init__(self, *, dados: Mapping[str, np.ndarray], procedencia: Procedencia,
                 embutidor: Callable[[np.ndarray], np.ndarray],
                 limiar_de_confianca: float | None = None) -> None:
        ...
        self._media_da_fonte = np.asarray(dados[f"mu_{procedencia.fonte_das_estatisticas}"],
                                          dtype=np.float64).reshape(-1)
        self._desvio_da_fonte = np.asarray(dados[f"sd_{procedencia.fonte_das_estatisticas}"],
                                           dtype=np.float64).reshape(-1)
        self._embutidor = embutidor
        self._limiar_de_confianca = float(
            procedencia.limiar_de_abstencao if limiar_de_confianca is None else limiar_de_confianca)
        if not 0.0 <= self._limiar_de_confianca <= 1.0:
            raise ErroDeClassificacao(
                f"limiar de confianca fora de [0,1]: {self._limiar_de_confianca}")
        self._classes = tuple(_classe_da_tampa(nome) for nome in self._nomes)  # D-28: recusa fora
    ...
    def identificacao(self) -> str:
        ...
        return (f"artefato:{self.procedencia.vintage}|extrator={self.procedencia.extrator}"
    ...
    @classmethod
    def abrir(cls, caminho: str | Path = ARTEFATO_PADRAO, *, fonte: str = FONTE_PADRAO,
              limiar_de_confianca: float | None = None,
              embutidor: Callable[[np.ndarray], np.ndarray] | None = None,
              meta_caminho: str | Path | None = None) -> ClassificadorDoArtefato:
        """Le o artefato e o json. Qualquer buraco na receita ou no metadado e erro declarado."""
        caminho = Path(caminho)
        meta = Path(meta_caminho) if meta_caminho is not None else caminho.with_suffix(".json")
        ...
        faltando = [c for c in ("vintage", "extrator", "limiar_abstencao") if c not in declarado]
        if faltando:
            raise ErroDeClassificacao(f"metadado sem {faltando} em {meta}: {sorted(declarado)}")
        ...
        for chave in (f"mu_{fonte}", f"sd_{fonte}"):
            if chave not in dados:
                fontes = sorted(c.removeprefix("mu_") for c in dados if c.startswith("mu_"))
                raise ErroDeClassificacao(
        ...
        if coef.ndim == 2 and coef.shape[0] != len(classes):
            problemas.append(f"{len(classes)} classes para {coef.shape[0]} linhas de coef")
        if intercepto.shape[0] != len(classes):
            problemas.append(f"intercept{intercepto.shape} para {len(classes)} classes")
        for chave in (f"mu_{fonte}", f"sd_{fonte}"):
            if np.asarray(dados[chave]).reshape(-1).shape[0] != (P.shape[1] if P.ndim == 2 else -1):
        ...
        if problemas:
            raise ErroDeClassificacao(f"artefato {caminho.name} inconsistente: " + "; ".join(problemas))

        limiar = float(declarado["limiar_abstencao"])
        if not 0.0 <= limiar <= 1.0:
            raise ErroDeClassificacao(f"limiar_abstencao do metadado fora de [0,1]: {limiar}")

        procedencia = Procedencia(
            caminho=str(caminho), sha256=_sha256(caminho), meta_caminho=str(meta),
            vintage=str(declarado["vintage"]), extrator=str(declarado["extrator"]),
            config_extrator=declarado.get("config_extrator") or {},
            limiar_de_abstencao=limiar, fonte_das_estatisticas=fonte, classes=classes,
            avaliacao=declarado.get("avaliacao") or {}, aviso=str(declarado.get("aviso") or ""),
            treino=declarado.get("treino"))

        if embutidor is None:
            embutidor = construir_embutidor(procedencia.config_extrator)

        return cls(dados=dados, procedencia=procedencia, embutidor=embutidor,
    ...
    def prever(self, recorte, dominio: Dominio, vista: Vista) -> Medida | None:  # noqa: D401
        """Medida da vista/dominio, ou `None` quando nao ha modelo para decidir (CORPO)."""
        return self.avaliar(recorte, dominio, vista)[1]

    def avaliar(self, recorte, dominio: Dominio,
                vista: Vista) -> tuple[np.ndarray, Medida | None]:
        ...
        if confianca < self._limiar_de_confianca:
            return probabilidades, Medida(
                vista=vista, dominio=dominio, classe=Classe.INCONCLUSIVO, confianca=confianca,
                qualidade=Qualidade.INSUFICIENTE, evidencias=self._evidencias(confianca, roteou=True))
        return probabilidades, Medida(
            vista=vista, dominio=dominio, classe=classe, confianca=confianca,
    ...
    def _evidencias(self, confianca: float, *, roteou: bool) -> tuple[Evidencia, ...]:
        ...
        fonte_do_artefato = f"{Path(self.procedencia.caminho).name}#{self.procedencia.sha256[:12]}"
        evidencias = [
            Evidencia(grandeza="probabilidade_da_classe_decidida", valor=confianca,
                      unidade="probabilidade", origem=Origem.CLASSIFICADOR,
                      papel=Papel.FALLBACK if roteou else Papel.DECIDE,
                      metodo=self.identificacao, fonte=fonte_do_artefato),
            Evidencia(grandeza="limiar_de_confianca_do_fallback", valor=self._limiar_de_confianca,
                      unidade="probabilidade", origem=Origem.CLASSIFICADOR, papel=Papel.FALLBACK,
        ...
        if self.procedencia.aviso:
            ...
            evidencias.append(Evidencia(grandeza="limitacao_declarada_pelo_artefato", valor=None,
                                        unidade="texto", origem=Origem.CLASSIFICADOR,
                                        papel=Papel.FALLBACK, metodo=self.procedencia.aviso,
                                        fonte=None))
