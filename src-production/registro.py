...
MAPA_CODIGO = {Classe.TAMPA_AUSENTE: "TAMPA_AUSENTE", Classe.DEFEITO_TAMPA: "TAMPA_DEFEITO"}
...
def qualidade_registro(laterais_decisorias: int, fora_da_janela: int = 0,
                       duplicadas: int = 0) -> str:
...
class Registro:
    ...
    def registrar(self, evento: Evento, fora_da_janela: tuple[Vista, ...] = (),
                  duplicadas: tuple[Vista, ...] = (),
                  referencias: tuple[ReferenciaDaEvidencia, ...] = ()) -> str:
        ...
        try:
            ...
            self._cx.execute(
                ...
                (evento.item_id, evento.capturado_em.isoformat(),
                 ...
                 ("vista_duplicada" if com_duplicata
                  else "timestamp_divergente" if divergente
                 ...
                 evento.equipamento, evento.localizacao, evento.versao_contrato, lote))
            ...
            for linha, classe, evidencias in self._linhas(evento):
                ...
                cur = self._cx.execute(
                    ...
                    (evento.item_id, linha.vista.value,
                     ...
                     ref.caminho if ref else None, ref.sha256 if ref else None))
    ...
    def registrar_gatilho(self, timestamp: str, estado: str, fonte: str = "nao_declarada",
                          ponto_id: int | None = None, item_id: str | None = None,
                          motivo: str | None = None, debounce_ms: int | None = None) -> int:
    ...
    def correcao_vigente(self, item_id: str) -> dict[str, object] | None:
        ...
        linha = self._cx.execute(
            ...
            " FROM correcao_operador WHERE item_id = ? ORDER BY id DESC LIMIT 1", (item_id,)).fetchone()
        return dict(linha) if linha else None

    def corrigir(self, item_id: str, decisao_corrigida: str, corrigido_por: str,
        ...
        linha = self._cx.execute("SELECT status_final FROM item WHERE item_id = ?",
        ...
        if not operador or len(operador) > 64 or any(caractere < " " for caractere in operador):
            raise EventoInvalido(
                "correcao sem operador identificado: informe de 1 a 64 caracteres imprimiveis")

        vigente = self.correcao_vigente(identificador)
        anterior = str(vigente["decisao_corrigida"]) if vigente else str(linha["status_final"] or "")
        if decisao == anterior:
            raise EventoInvalido(
        ...
        self._cx.commit()
        return {"id": int(cur.lastrowid), "item_id": identificador, "decisao_original": anterior,
                "decisao_corrigida": decisao, "corrigido_por": operador, "timestamp": instante}

    def vincular_gatilho_a_item(self, gatilho_id: int, item_id: str) -> None:
        """Liga o gatilho ao item que ele abriu, depois que o item existe e esta gravado."""
        if self.ler(item_id) is None:
            raise EventoInvalido(f"item inexistente: {item_id!r}")
        cur = self._cx.execute("UPDATE evento_gatilho SET item_id = ? WHERE id = ?",
                               (item_id, gatilho_id))
        if cur.rowcount != 1:
    ...
    def _garantir_lote(self, evento: Evento) -> str | None:
        ...
        try:
            ...
            return None
        self._cx.execute("INSERT OR IGNORE INTO lote (lote_id, data_inicio) VALUES (?,?)",
                         (lote, evento.capturado_em.date().isoformat()))
        return lote

    def _gravar_evidencias(self, inspecao_vista_id: int, evidencias: tuple[Evidencia, ...]) -> None:
        for e in evidencias:
            self._cx.execute(
                "INSERT INTO evidencia (inspecao_vista_id, grandeza, valor, unidade, origem, papel,"
                " metodo, fonte) VALUES (?,?,?,?,?,?,?,?)",
                (inspecao_vista_id, e.grandeza, e.valor, e.unidade, e.origem.value, e.papel.value,
                 e.metodo, e.fonte))

    def _linhas(self, evento: Evento) -> tuple[tuple[LinhaDeVista, Classe | None, tuple[Evidencia, ...]], ...]:
        ...
        for m in evento.medidas:
            pares.append((LinhaDeVista(vista=m.vista, dominio=m.dominio, papel=papel_da_linha(m.vista, m),
                                       ...
                                       confianca=m.confianca), m.classe, tuple(m.evidencias)))
    ...
    def ler(self, item_id: str) -> ItemGravado | None:
        ...
        for row in rows:
            ...
            referencia = ReferenciaDaEvidencia(vista=vista, caminho=row["caminho_evidencia"],
                                                sha256=row["sha256_evidencia"])
            anterior = referencias_por_vista.get(vista)
            if anterior is not None and anterior != referencia:
                raise ErroDeRegistro(f"referencias divergentes gravadas para a vista {vista.value}")
    ...
    def _mesma_evidencia(self, gravado: ItemGravado, evento: Evento,
                         referencias: tuple[ReferenciaDaEvidencia, ...]) -> bool:
        """Reenvio so e repetido se linhas E referencias gravadas forem identicas."""
        linhas = tuple(l for l, _, _ in self._linhas(evento))
        return (_chave(gravado.vistas) == _chave(linhas)
                and _chave_referencias(gravado.referencias) == _chave_referencias(referencias))
