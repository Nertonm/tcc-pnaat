...
def completo(tampa, corpo, conf=0.92):
    return (
        medida(Vista.LATERAL1, Dominio.TAMPA, tampa, conf),
...
def main() -> int:
    ...
    cx = reg._cx
    cx.execute(
        "INSERT INTO lote (lote_id, data_inicio, turno) VALUES ('L2026-09-15-A','2026-09-15','manha')"
    )
    cx.execute(
        "INSERT INTO lote (lote_id, data_inicio, turno) VALUES ('L2026-09-15-B','2026-09-15','tarde')"
    )
    cx.execute(
    ...
    itens = [
        (
            ...
            0,
            Classe.NORMAL,
            completo(Classe.NORMAL, Classe.NORMAL),
            "lateral1",
        ),
        (
            "ITM-002",
        ...
        (
            ...
            completo(Classe.TAMPA_AUSENTE, Classe.NORMAL),
            "lateral1",
        ),
        (
            "ITM-004",
            ...
            completo(Classe.DEFEITO_TAMPA, Classe.NORMAL),
            "lateral2",
        ),
        (
            "ITM-005",
        ...
        (
            ...
            26,
            Classe.NORMAL,
            (medida(Vista.LATERAL1, Dominio.TAMPA, Classe.NORMAL, 0.7),),
            None,
        ),
    ]
    for item_id, lote, segundos, classe, medidas, vista_evidencia in itens:
        reg.registrar(
            Evento(
                item_id=item_id,
                capturado_em=T0 + timedelta(seconds=segundos),
                equipamento="pi5-rig",
        ...
        cx.execute(
            "UPDATE item SET lote_id=?, fonte_trigger='e18_d80nk', velocidade_rig_mm_s=100.0"
            " WHERE item_id=?",
            (lote, item_id),
        )
    ...
    for item_id, vista, texto, cor in (
        ...
        cx.execute(
            "UPDATE inspecao_vista SET caminho_evidencia=?, latencia_ms=?, pixels_saturados_pct=?"
    ...
    for i, (seg, estado, item, motivo) in enumerate(
        [
            ...
            (8, "aceito", "ITM-003", None),
            (12, "aceito", "ITM-004", None),
            (20, "aceito", "ITM-005", None),
            (26, "aceito", "ITM-006", None),
            (30, "falso", None, "sem captura na janela"),
            (34, "duplicado", None, "debounce"),
        ]
    ):
        reg.registrar_gatilho(
            (T0 + timedelta(seconds=seg)).isoformat(),
            estado,
            fonte="e18_d80nk",
            item_id=item,
            ponto_id=1,
    ...
    cx.execute(
        "INSERT INTO correcao_operador (item_id, decisao_original, decisao_corrigida,"
    ...
    reg.fechar()
    print(
        f"banco de demonstracao: {DESTINO} | itens={total} linhas_de_vista={vistas} "
