...
# --- ROI por camera (derivada das caixas anotadas; ver dataset/TRABALHO/roi_por_camera.py)
ROI_POR_CAMERA = {}


def _carrega_roi() -> dict:
    from pathlib import Path as _Path          # import tardio: no topo do modulo Path pode nao existir
    try:
        dados = json.loads(_Path(os.environ.get('PNAAT_ROI_JSON', '/opt/pnaat-vision/roi.json')).read_text())
        return {cam: v.get('roi_normalizada') for cam, v in (dados.get('cameras') or {}).items()
                if v.get('roi_normalizada')}
    except Exception as exc:  # sem roi.json o pipeline segue com a imagem inteira
        print(f'[roi] sem configuracao ({exc}); seguindo com frame inteiro')
        return {}


def _recorta_roi(destino: Path, camera: str) -> None:
    """Recorta a ROI da camera no arquivo ja gravado (roda depois da rotacao)."""
    caixa = ROI_POR_CAMERA.get(camera)
    if not caixa:
        return
    try:
        ...
        with Image.open(destino) as im:
            ...
            x = int(caixa['x'] * largura)
            y = int(caixa['y'] * altura)
            w = int(caixa['w'] * largura)
            h = int(caixa['h'] * altura)
            if w > 16 and h > 16 and (w < largura or h < altura):
                im.crop((x, y, x + w, y + h)).save(destino, format='JPEG', quality=95, subsampling=0)
    except Exception as exc:
        print(f'[roi] falha ao recortar {camera}: {exc}')


_ROI_CARREGADA = False


def _camera_do_nome(destino: Path):
    """A câmera vem do nome do arquivo; só JPEG entra (json passa direto)."""
    if destino.suffix.lower() not in ('.jpg', '.jpeg'):
        return None
    nome = destino.name.lower()
    for marca, camera in (('acerola-csi', 'csi'), ('acerola-usb', 'usb'),
                          ('espcam', 'espcam'), ('foto-', 'csi')):
        if marca in nome:
            return camera
    return None


def _aplica_roi_no_arquivo(destino: Path) -> None:
    """Recorta a ROI da câmera no arquivo já gravado (depois da rotação)."""
    global _ROI_CARREGADA
    if not _ROI_CARREGADA:
        ROI_POR_CAMERA.update(_carrega_roi())
        _ROI_CARREGADA = True
    camera = _camera_do_nome(destino)
    if not camera:
        return
    caixa = ROI_POR_CAMERA.get(camera)
    if not caixa:
    ...
    try:
        ...
        with Image.open(destino) as im:
            largura, altura = im.size
            x, y = int(caixa['x'] * largura), int(caixa['y'] * altura)
            w, h = int(caixa['w'] * largura), int(caixa['h'] * altura)
            if w <= 16 or h <= 16 or (w >= largura and h >= altura):
                return
            recorte = im.crop((x, y, x + w, y + h))
        temporario = destino.with_name(destino.name + '.roi.tmp')
        recorte.save(temporario, format='JPEG', quality=95, subsampling=0)
        temporario.replace(destino)
    except Exception as exc:
        print(f'[roi] falha em {destino.name}: {exc}')
import subprocess
import threading
...
ESP_CAM_FRAME_URL = os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/frame.jpg"
...
SERIES_DIR = Path(os.environ.get("PNAAT_SERIES_DIR", "series-3-cameras")).resolve()
# em producao declare PNAAT_SERIES_DIR com o diretorio real das series; sem env, o default e relativo
# ao cwd (o teste de rotas e os fluxos sem hardware usam esse default e sobrescrevem antes de rodar)
...
PREVIEW_DIR = Path(os.environ.get('PNAAT_RIG_PREVIEW', '/dev/shm/pnaat-dataset-preview'))
...
def _gravar_atomico(destino: Path, data: bytes) -> None:
    ...
    try:
        ...
        os.close(dirfd)
    _aplica_roi_no_arquivo(destino)
...
def _capturar_esp_novo(destino: Path) -> None:
    ...
    from urllib.request import Request, urlopen
    with urlopen(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/status", timeout=5) as resp:
        antes = json.loads(resp.read().decode())["frames_ok"]
    with urlopen(Request(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/capture", method="POST"), timeout=5):
        pass
    limite = time.time() + 45
...
def registrar_foto_trigger(trigger_n: int, trigger_em: float) -> dict:
    ...
    try:
        ...
        from urllib.request import Request, urlopen
        with urlopen(Request(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/frame.jpg"), timeout=8) as resp:
            jpeg = resp.read()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/teste-trigger-3-cameras":
            ...
            try:
                req = Request(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/simular?n=%d" % ref, method="POST")
        ...
        if u.path == "/testar-delay":
            from urllib.request import Request, urlopen
            try:
                with urlopen(Request(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/teste-delay", method="POST"), timeout=35) as resp:
        ...
        if u.path == "/configurar-delay":
            ...
            from urllib.request import Request, urlopen
            try:
                req = Request(os.environ.get("PNAAT_PONTE", "http://127.0.0.1:8094").rstrip("/") + "/delay?ms=%d" % ms, method="POST")
                with urlopen(req, timeout=5) as resp:
                    resposta = resp.read(256).decode("utf-8", "replace")
