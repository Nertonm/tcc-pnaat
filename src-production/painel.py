...
class Painel:
    ...
    def taxa_de_defeito_por_lote(self) -> tuple[TaxaDeDefeito, ...]:
        linhas = self._cx.execute(
            ...
            " FROM item GROUP BY lote ORDER BY lote"
        ).fetchall()
        return tuple(
            TaxaDeDefeito(
                r["lote"],
    ...
    def defeitos_frequentes(
        self, de: str | None = None, ate: str | None = None
    ) -> tuple[DefeitoFrequente, ...]:
        sql = (
            "SELECT v.codigo_defeito AS codigo, t.severidade AS sev, COUNT(*) AS n"
        ...
        return tuple(
            DefeitoFrequente(r["codigo"], r["sev"], r["n"])
            for r in self._cx.execute(sql, par)
        )

    # -------------------------------------------------------------- 3

    def correlacao_ambiental(
        self, janela_s: int = 30, variavel: str = "temperatura"
    ) -> CorrelacaoAmbiental:
        ...
        itens = self._cx.execute(
            "SELECT item_id, timestamp_trigger, (status_final='defeito') AS y FROM item"
        ).fetchall()
        ...
        for it in itens:
            amb = self._cx.execute(
                ...
                f"{variavel} IS NOT NULL",
                (it["timestamp_trigger"], janela_s),
            ).fetchall()
            for a in amb:
    ...
    def tendencia_por_hora(self) -> tuple[TendenciaPorHora, ...]:
        linhas = self._cx.execute(
            ...
            " SUM(status_final='defeito') AS defeitos FROM item GROUP BY hora ORDER BY hora"
        ).fetchall()
        return tuple(
            TendenciaPorHora(
                r["hora"],
    ...
    def saude_dos_nos(self, janela_h: int = 1) -> tuple[SaudeDoNo, ...]:
        ...
        return tuple(
            SaudeDoNo(r["ponto_id"], r["timestamp"], r["status"], r["fila_pendente"])
            for r in linhas
        )
    ...
    def latencia_por_vista(self) -> tuple[LatenciaPorVista, ...]:
        linhas = self._cx.execute(
            ...
            " FROM inspecao_vista WHERE latencia_ms IS NOT NULL GROUP BY vista ORDER BY vista"
        ).fetchall()
        return tuple(
            LatenciaPorVista(r["vista"], r["n"], r["media"], r["maxima"])
            for r in linhas
        )
    ...
    def correcoes_do_item(self, item_id: str) -> tuple[object, ...]:
        ...
        linhas = self._cx.execute(
            ...
            " FROM correcao_operador WHERE item_id = ? ORDER BY id",
            (item_id,),
        ).fetchall()
        return tuple(
            CorrecaoParaAuditoria(
                r["item_id"],
                r["decisao_original"],
                r["decisao_corrigida"],
                r["corrigido_por"],
    ...
    def correcoes_para_auditoria(self) -> tuple[CorrecaoParaAuditoria, ...]:
        return tuple(
            ...
            for r in self._cx.execute(
                "SELECT * FROM correcao_operador ORDER BY timestamp"
            )
        )
    ...
    def separacoes_nao_confirmadas(self) -> tuple[SeparacaoNaoConfirmada, ...]:
        return tuple(
            SeparacaoNaoConfirmada(
                r["item_id"], r["status"], r["tentativa"], r["via_sensor"]
            )
            for r in self._cx.execute(
    ...
    def gatilho_por_fonte(self) -> tuple[GatilhoPorFonte, ...]:
        ...
        linhas = self._cx.execute(
            ...
            " FROM evento_gatilho GROUP BY fonte ORDER BY eventos DESC"
        ).fetchall()
        return tuple(
            GatilhoPorFonte(
                r["fonte"],
    ...
    def perda_de_deteccao(self) -> PerdaDeDeteccao:
        ...
        return PerdaDeDeteccao(
            False, "exige referencia externa (contagem do encoder KY-040, D-21)"
        )
    ...
    def saturacao_por_vista(self) -> tuple[SaturacaoPorVista, ...]:
        linhas = self._cx.execute(
            ...
            " WHERE pixels_saturados_pct IS NOT NULL GROUP BY vista ORDER BY vista"
        ).fetchall()
        return tuple(
            SaturacaoPorVista(r["vista"], r["n"], r["media"], r["maxima"])
            for r in linhas
        )
    ...
    def inconclusivos_por_lote(self) -> tuple[InconclusivoPorLote, ...]:
        linhas = self._cx.execute(
            ...
            " FROM item WHERE status_final = 'inconclusivo' GROUP BY lote, motivo ORDER BY n DESC"
        ).fetchall()
        return tuple(
            InconclusivoPorLote(r["lote"], r["motivo"], r["n"]) for r in linhas
        )
    ...
    def discordancia_lateral(self) -> DiscordanciaLateral:
        r = self._cx.execute(
            ...
            "     AND v.dominio IS NOT NULL) AS n_med FROM item i)"
        ).fetchone()
        pares = r["pares"] or 0
        disc = r["disc"] or 0
    ...
    def aprovados(self) -> int:
        ...
        return int(
            self._cx.execute(
                "SELECT COUNT(*) FROM item WHERE status_final = 'ok'"
            ).fetchone()[0]
        )
