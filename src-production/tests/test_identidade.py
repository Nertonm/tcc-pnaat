...
from identidade import (FORMATO, ErroDeIdentidade, SequenciaDeItens, decompor, montar,
                        validar_lote)
...
def _evento(item_id: str) -> Evento:
    return Evento(item_id=item_id, capturado_em=AGORA, equipamento="pi5-rig", localizacao="bancada-b",
                  vistas=(Vista.TOPO,), status=Classe.NORMAL,
                  medidas=(Medida(vista=Vista.LATERAL1, dominio=Dominio.TAMPA, classe=Classe.NORMAL,
                                  confianca=0.9),
                           Medida(vista=Vista.LATERAL2, dominio=Dominio.TAMPA, classe=Classe.NORMAL,
                                  confianca=0.9)))
...
def test_reboot_nao_reinicia_a_sequencia(tmp_path):
    ...
    assert sorted(r["item_id"] for r in reg2._cx.execute("SELECT item_id FROM item")) == [
        "L1-000001", "L1-000002"]
    reg2.fechar()
...
def test_falha_dentro_da_reserva_nao_grava_e_libera_o_numero(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        with seq.reservar("L1") as item_id:
            assert item_id == "L1-000001"
