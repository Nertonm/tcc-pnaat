...
def test_delay_csi_uses_previous_value_from_rig_not_esp_bridge(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        api, "_chamar_ponte", lambda path: {"trigger": {"delay_ms": 1500}}
    )
    r = api._rota_rig_delay(
        _ctx(tmp_path), {"ms": 300, "camera": "csi", "operador": "teste"}
    )
...
def test_delay_all_is_explicitly_accepted(monkeypatch, tmp_path):
    monkeypatch.setattr(
        api, "_chamar_ponte", lambda path: {"trigger": {"delay_ms": 900}}
    )
    monkeypatch.setattr(
        api,
        "_chamar_rig",
        lambda path, timeout=0: {
            "delay_por_camera_ms": {"csi": 900, "usb": 900, "espcam": 900}
        },
    )
    r = api._rota_rig_delay(
        _ctx(tmp_path), {"ms": 900, "camera": "todas", "operador": "teste"}
    )
