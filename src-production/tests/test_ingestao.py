...
from ingerir_serie import ingerir
...
class ClassificadorFalso:
    ...
    def prever(self, recorte, dominio: Dominio, vista: Vista):
        return Medida(vista=vista, dominio=dominio, classe=Classe.NORMAL, confianca=0.99,
...
def serie(tmp_path) -> Path:
    ...
    fontes = []
    for indice, (camera, nome) in enumerate((("camera-1", "camera-1.jpg"),
    ...
    (pasta / "manifest.json").write_text(json.dumps({
        "serie": pasta.name, "capturado_em": "2026-09-14 19:10:45", "trigger_n": 712,
        "trigger_em": 1789417845.97, "captura_inicio_epoch": 1789417845.97,
        "captura_fim_epoch": 1789417850.67, "delay_trigger_ate_fim_ms": 4699, "fontes": fontes,
...
def _banco(tmp_path) -> Path:
    ...
    registro._cx.execute("INSERT INTO lote (lote_id, data_inicio) VALUES ('L1','2026-09-15')")
    registro._cx.commit()
    registro.fechar()
    return caminho
...
def test_ingere_serie_completa_com_as_tres_vistas(serie, tmp_path):
    ...
    resultado = ingerir(serie, lote="L1", db=banco, roi=(0.0, 0.0, 1.0, 1.0), janela_ms=800,
                        alinhamento_declarado=True, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)
    ...
    com_prova = {l["vista"] for l in linhas if l["caminho_evidencia"] and l["sha256_evidencia"]}
...
def test_sem_janela_declarada_nada_e_utilizavel(serie, tmp_path):
    ...
    banco = _banco(tmp_path)
    resultado = ingerir(serie, lote="L1", db=banco, roi=(0.0, 0.0, 1.0, 1.0), janela_ms=None,
                        alinhamento_declarado=True, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)
...
def test_alinhamento_nao_verificado_nao_vira_evidencia(serie, tmp_path):
    banco = _banco(tmp_path)
    resultado = ingerir(serie, lote="L1", db=banco, roi=(0.0, 0.0, 1.0, 1.0), janela_ms=800,
                        alinhamento_declarado=False, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)
...
def test_duas_cameras_para_a_mesma_vista_no_manifesto_e_erro():
    manifesto = {"fontes": [{"camera": "camera-1", "nome": "a.jpg"},
...
def test_serie_sem_manifesto_e_recusada(tmp_path):
    ...
    with pytest.raises(ErroDeMapeamento, match="manifest.json"):
        ingerir(pasta, lote="L1", db=_banco(tmp_path), roi=(0.0, 0.0, 1.0, 1.0), janela_ms=800,
                alinhamento_declarado=True, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)
...
def test_vincula_o_evento_de_gatilho_ao_item(serie, tmp_path):
    ...
    registro = Registro.abrir(banco)
    try:
        gatilho = registro.registrar_gatilho(datetime.now(UTC).isoformat(timespec="seconds"), "aceito",
                                             fonte="e18_d80nk", motivo="teste de ingestao")
    finally:
        registro.fechar()
    ...
    registro = Registro.abrir(banco)
    try:
        linha = registro._cx.execute("SELECT item_id FROM evento_gatilho WHERE id = ?",
                                     (gatilho,)).fetchone()
    finally:
        registro.fechar()
...
def test_id_sai_da_sequencia_do_lote(serie, tmp_path):
    ...
    """
    banco = _banco(tmp_path)
    primeiro = ingerir(serie, lote="L1", db=banco, roi=(0.0, 0.0, 1.0, 1.0), janela_ms=800,
                       alinhamento_declarado=True, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)
    segundo = ingerir(serie, lote="L1", db=banco, roi=(0.0, 0.0, 1.0, 1.0), janela_ms=800,
                      alinhamento_declarado=True, classificador=ClassificadorFalso(), mapa_texto=MAPA_DO_TESTE)

    assert primeiro["item_id"] != segundo["item_id"]
    assert primeiro["item_id"].endswith("000001") and segundo["item_id"].endswith("000002")
