...
from conformidade import (MOTIVO_CHECK_AUSENTE, MOTIVO_CHECK_ESCALONADO, MOTIVO_CHECK_VIOLADO,
                   MOTIVO_RIG_INCOMPLETO, ErroDeConformidade, ConfiguracaoDoRig, avaliar_conformidade)
...
def _par(dominio, c1, c2, q1=Qualidade.OK, q2=Qualidade.OK, conf1=0.9, conf2=0.9):
    return (_m(Vista.LATERAL1, dominio, c1, conf1, q1), _m(Vista.LATERAL2, dominio, c2, conf2, q2))
...
def _completo(tampa, corpo, topo=TOPO_OK, conf_tampa=(0.9, 0.9), conf_corpo=(0.9, 0.9)):
    return (_par(Dominio.TAMPA, *tampa, conf1=conf_tampa[0], conf2=conf_tampa[1])
            + _par(Dominio.CORPO, *corpo, conf1=conf_corpo[0], conf2=conf_corpo[1]) + (topo,))
...
def test_aprovacao_exige_todos_os_dominios_normais_e_check_presente():
    r = avaliar_conformidade(_completo((Classe.NORMAL, Classe.NORMAL), (Classe.NORMAL, Classe.NORMAL)))
    assert r.status == "ok" and not r.discordancia_lateral and not r.check_escalonado
    assert {d.dominio: d.status for d in r.por_dominio} == {Dominio.TAMPA: "ok", Dominio.CORPO: "ok"}
...
def test_vista_unica_nao_aprova_o_dominio():
    medidas = (_m(Vista.LATERAL1, Dominio.TAMPA, Classe.NORMAL),
               _m(Vista.LATERAL1, Dominio.CORPO, Classe.NORMAL),
               _m(Vista.LATERAL2, Dominio.CORPO, Classe.NORMAL), TOPO_OK)
    r = avaliar_conformidade(medidas)
    assert r.status == "inconclusivo"
    tampa = _por_dominio(r)[Dominio.TAMPA]
    assert tampa.status == "inconclusivo" and "vistas_insuficientes_tampa" in tampa.motivos
...
def test_defeito_em_uma_vista_reprova_e_marca_discordancia():
    r = avaliar_conformidade(_completo((Classe.TAMPA_MAL_ROSQUEADA, Classe.NORMAL), (Classe.NORMAL, Classe.NORMAL)))
    assert r.status == "defeito" and r.discordancia_lateral
    tampa = _por_dominio(r)[Dominio.TAMPA]
    assert tampa.classe is Classe.TAMPA_MAL_ROSQUEADA
    assert "classes_divergentes_tampa" in tampa.motivos
    assert tampa.origem == ((Vista.LATERAL1, Classe.TAMPA_MAL_ROSQUEADA), (Vista.LATERAL2, Classe.NORMAL))


def test_defeito_nos_dois_dominios_reprova():
    r = avaliar_conformidade(_completo((Classe.TAMPA_AUSENTE, Classe.TAMPA_AUSENTE),
                         (Classe.DEFORMIDADE, Classe.DEFORMIDADE)))
    assert r.status == "defeito"
    assert {d.dominio: d.classe for d in r.por_dominio} == {Dominio.TAMPA: Classe.TAMPA_AUSENTE,
                                                           Dominio.CORPO: Classe.DEFORMIDADE}


def test_precedencia_vence_a_confianca():
    """tampa_ausente precede tampa_mal_rosqueada, mesmo com confianca menor."""
    r = avaliar_conformidade(_completo((Classe.TAMPA_MAL_ROSQUEADA, Classe.TAMPA_AUSENTE),
                         (Classe.NORMAL, Classe.NORMAL), conf_tampa=(0.99, 0.70)))
    assert _por_dominio(r)[Dominio.TAMPA].classe is Classe.TAMPA_AUSENTE


def test_mesma_classe_com_confiancas_diferentes():
    r = avaliar_conformidade(_completo((Classe.TAMPA_AUSENTE, Classe.TAMPA_AUSENTE), (Classe.NORMAL, Classe.NORMAL),
                         conf_tampa=(0.60, 0.98)))
    tampa = _por_dominio(r)[Dominio.TAMPA]
    assert tampa.classe is Classe.TAMPA_AUSENTE
    assert tampa.origem == ((Vista.LATERAL1, Classe.TAMPA_AUSENTE),
                            (Vista.LATERAL2, Classe.TAMPA_AUSENTE))
    assert not r.discordancia_lateral


def test_inconclusivo_em_uma_vista_nao_aprova():
    r = avaliar_conformidade(_completo((Classe.NORMAL, Classe.INCONCLUSIVO), (Classe.NORMAL, Classe.NORMAL)))
    assert r.status == "inconclusivo"
...
def test_topo_nao_cancela_defeito_das_laterais():
    medidas = _completo((Classe.TAMPA_AUSENTE, Classe.TAMPA_AUSENTE), (Classe.NORMAL, Classe.NORMAL))
...
def test_check_obrigatorio_ausente_nao_aprova():
    medidas = _par(Dominio.TAMPA, Classe.NORMAL, Classe.NORMAL) + _par(Dominio.CORPO, Classe.NORMAL, Classe.NORMAL)
    r = avaliar_conformidade(medidas)
...
def test_vista_fora_do_rig_declarado_e_erro():
    ...
    with pytest.raises(ErroDeConformidade):
        avaliar_conformidade(medidas + (_m(Vista.TOPO, Dominio.CORPO, Classe.NORMAL),),
               config=ConfiguracaoDoRig(vista_check=Vista.TOPO, vistas_decisorias=(Vista.LATERAL1,)))
...
def test_configuracao_menor_nao_aprova():
    ...
    medidas = (_m(Vista.LATERAL1, Dominio.TAMPA, Classe.NORMAL),
               _m(Vista.LATERAL1, Dominio.CORPO, Classe.NORMAL), TOPO_OK)
    r = avaliar_conformidade(medidas, config=ConfiguracaoDoRig(vistas_decisorias=(Vista.LATERAL1,)))
