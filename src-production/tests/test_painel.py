...
def _completo(vista_lat1, vista_lat2, corpo=Classe.NORMAL):
    return (
        _m(Vista.LATERAL1, Dominio.TAMPA, vista_lat1),
...
def painel(tmp_path):
    ...
    cx.execute(
        "UPDATE item SET lote_id='L1', fonte_trigger='e18_d80nk' WHERE item_id IN ('i-A','i-B','i-C')"
    )
    cx.execute(
        "UPDATE item SET lote_id='L2', fonte_trigger='e18_d80nk' WHERE item_id='i-D'"
    )
    # campos que o registro ainda nao produz: latencia, saturacao, evidencia, ambiente, heartbeat
    cx.execute(
        "UPDATE inspecao_vista SET latencia_ms=40, pixels_saturados_pct=1.5 WHERE vista='lateral1'"
    )
    cx.execute(
        "UPDATE inspecao_vista SET latencia_ms=60, pixels_saturados_pct=0.5 WHERE vista='lateral2'"
    )
    cx.execute(
    ...
    for i, (dt, temp) in enumerate(
        ...
        cx.execute(
            "INSERT INTO evento_ambiental (timestamp, ponto_id, temperatura) VALUES (?,?,?)",
            ((dt.isoformat()), 1, temp),
        )
    cx.execute(
        "INSERT INTO heartbeat_no (ponto_id, timestamp, status, fila_pendente, latencia_envio_ms)"
        " VALUES (1, ?, 'online', 0, 12)",
        (datetime.now(timezone.utc).isoformat(),),
    )
    cx.execute(
        "INSERT INTO heartbeat_no (ponto_id, timestamp, status, fila_pendente)"
        " VALUES (1, ?, 'offline', 3)",
        ((datetime.now(timezone.utc) - timedelta(hours=5)).isoformat(),),
    )
    cx.execute(
        "INSERT INTO correcao_operador (item_id, decisao_original, decisao_corrigida, corrigido_por, timestamp)"
        " VALUES ('i-B','defeito_tampa','normal','operador-1','2026-09-12T23:30:00+00:00')"
    )
    cx.execute(
        "INSERT INTO evento_rejeicao (item_id, timestamp_ordenado, status_ordem, status, via_sensor)"
        " VALUES ('i-C','2026-09-13T00:00:00+00:00','emitida','timeout','e18_d80nk')"
    )
    # eventos de gatilho (RF-01.1): o falso e o duplicado existem so aqui, nao no item
...
def test_defeitos_frequentes_com_severidade(painel):
    ...
    assert (
        d["TAMPA_DEFEITO"].severidade == "major" and d["TAMPA_DEFEITO"].ocorrencias == 1
    )
    assert (
        d["TAMPA_AUSENTE"].severidade == "critico"
        and d["TAMPA_AUSENTE"].ocorrencias == 2
    )
...
def test_itens_criticos_traz_evidencia(painel):
    ...
    assert len(c) == 2 and {i.item_id for i in c} == {
        "i-C"
    }  # uma linha por lateral do item
...
def test_latencia_por_vista(painel):
    ...
    assert (l["lateral1"].medidas, l["lateral1"].media_ms, l["lateral1"].maxima_ms) == (
        7,
        40.0,
        40,
    )
...
def test_gatilho_por_fonte_com_falso_e_duplicado(painel):
    g = {r.fonte: r for r in painel.gatilho_por_fonte()}
    e18 = g["e18_d80nk"]
    assert (e18.eventos, e18.aceitos, e18.falsos, e18.duplicados) == (4, 2, 1, 1)
    assert e18.taxa_falso == pytest.approx(0.25)
    assert g["vl53l0x"].eventos == 1 and g["vl53l0x"].taxa_falso == 0.0


def test_perda_de_deteccao_se_declara_nao_instrumentada(painel):
    """Item que passou sem disparar nao e observavel pelo gatilho: exige o encoder."""
    p = painel.perda_de_deteccao()
    assert p.instrumentada is False
    assert "encoder" in p.motivo
...
def test_saturacao_por_vista(painel):
    ...
    assert (
        s["lateral1"].medidas,
        s["lateral1"].media_pct,
        s["lateral1"].maxima_pct,
    ) == (7, 1.5, 1.5)
...
def test_inconclusivos_por_lote_e_motivo(painel):
    ...
    assert (
        len(i) == 1 and i[0].lote_id == "L2" and i[0].motivo == "vista_lateral_ausente"
    )


def test_distribuicao_por_dominio(painel):
    d = {
        (r.dominio, r.estado): r.ocorrencias for r in painel.distribuicao_por_dominio()
    }
    # i-D tem UMA lateral na tampa: dominio inconclusivo, nao ok (D-04/D-29)
    assert (
        d[("tampa", "defeito")] == 2
        and d[("tampa", "ok")] == 1
        and d[("tampa", "inconclusivo")] == 1
    )
