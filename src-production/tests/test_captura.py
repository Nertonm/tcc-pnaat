...
from captura import (
    ...
    MOTIVO_JANELA_NAO_DECLARADA,
    VISTAS_ESPERADAS,
    Alinhamento,
    ErroDeCaptura,
    FonteDeDiretorio,
    ItemCapturado,
    VerificadorPorTemplate,
    VistaCapturada,
)
...
def test_template_aprova_o_enquadramento_de_referencia(template):
    alinhamento, deslocamento = VerificadorPorTemplate(template, POSICAO_REF).verificar(
        _quadro()
    )
...
def test_deslocamento_dentro_da_tolerancia_aprova(template):
    alinhamento, deslocamento = VerificadorPorTemplate(
        template, POSICAO_REF, tolerancia_px=10
    ).verificar(_quadro(dx=6))
...
def test_gabarito_ausente_e_fora_da_tolerancia(template):
    alinhamento, deslocamento = VerificadorPorTemplate(template, POSICAO_REF).verificar(
        np.full((ALTURA, LARGURA), 40, np.uint8)
    )
    assert alinhamento is Alinhamento.FORA_DA_TOLERANCIA and deslocamento == float(
        "inf"
    )
...
def test_vista_dentro_da_janela_e_utilizavel(tmp_path, template):
    fonte = FonteDeDiretorio(
        tmp_path,
        verificador=VerificadorPorTemplate(template, POSICAO_REF),
        janela_s=1.0,
    )
    _escrever(
        tmp_path / "i-1", Vista.LATERAL1, _quadro(), AGORA + timedelta(milliseconds=300)
    )
...
def test_vista_fora_da_janela_conta_como_faltante(tmp_path, template):
    ...
    _escrever(tmp_path / "i-1", Vista.LATERAL1, _quadro(dx=1))  # na hora
    _escrever(
        tmp_path / "i-1", Vista.LATERAL2, _quadro(dx=2), AGORA + timedelta(seconds=5)
    )  # atrasada
    item = fonte.capturar("i-1", AGORA)
    assert [v.vista for v in item.vistas_utilizaveis] == [Vista.LATERAL1]
...
def test_sem_janela_declarada_nada_e_utilizavel(tmp_path, template):
    ...
    fonte = FonteDeDiretorio(
        tmp_path, verificador=VerificadorPorTemplate(template, POSICAO_REF)
    )
...
def test_sem_verificador_nada_e_utilizavel(tmp_path):
    ...
    for i, v in enumerate(VISTAS_ESPERADAS):
        _escrever(tmp_path / "i-1", v, _quadro(dx=i))  # conteudo distinto por vista
    item = fonte.capturar("i-1", AGORA)
...
def test_com_verificador_so_o_alinhado_e_utilizavel(tmp_path, template):
    ...
    _escrever(
        tmp_path / "i-1", Vista.LATERAL1, _quadro(dx=1)
    )  # cada vista com conteudo proprio:
    _escrever(
        tmp_path / "i-1", Vista.LATERAL2, _quadro(dx=70)
    )  # imagens identicas sao duplicata
    _escrever(tmp_path / "i-1", Vista.TOPO, _quadro(dy=1))
    item = fonte.capturar("i-1", AGORA)
    assert {v.vista for v in item.vistas_utilizaveis} == {Vista.LATERAL1, Vista.TOPO}
    assert item.duplicadas == ()
...
def test_vista_repetida_e_recusada():
    v = VistaCapturada(
        vista=Vista.TOPO, imagem="x.jpg", capturado_em=AGORA, alinhamento=Alinhamento.OK
    )
    with pytest.raises(ErroDeCaptura):
...
def test_trigger_sem_fuso_e_recusado():
    with pytest.raises(ErroDeCaptura):
        ItemCapturado(
            item_id="i-1",
            trigger_em=datetime(2026, 9, 13),  # noqa: DTZ001
            vistas=(
                VistaCapturada(
                    vista=Vista.TOPO,
                    imagem="x.jpg",
...
def test_duas_evidencias_para_a_mesma_vista_e_ambigua(tmp_path, template):
    fonte = FonteDeDiretorio(
        tmp_path,
        verificador=VerificadorPorTemplate(template, POSICAO_REF),
        janela_s=5.0,
    ...
    (tmp_path / "i-1" / "lateral1.png").write_bytes(
        (tmp_path / "i-1" / "lateral1.jpg").read_bytes()
    )
    item = fonte.capturar("i-1", AGORA)
    assert [v.vista for v in item.duplicadas] == [Vista.LATERAL1]
    assert item.vistas_utilizaveis == ()  # ambigua nao vira evidencia


def test_mesma_imagem_em_duas_vistas_e_duplicata(tmp_path, template):
    """Concordancia falsa entre as laterais: as duas sao descartadas."""
    fonte = FonteDeDiretorio(
        tmp_path,
        verificador=VerificadorPorTemplate(template, POSICAO_REF),
        janela_s=5.0,
    ...
    _escrever(tmp_path / "i-1", Vista.LATERAL1, _quadro())
    _escrever(tmp_path / "i-1", Vista.LATERAL2, _quadro())  # byte a byte igual
    _escrever(tmp_path / "i-1", Vista.TOPO, _quadro(dx=3))
    item = fonte.capturar("i-1", AGORA)
    assert {v.vista for v in item.duplicadas} == {Vista.LATERAL1, Vista.LATERAL2}
    assert [v.vista for v in item.vistas_utilizaveis] == [Vista.TOPO]


def test_imagens_distintas_nao_sao_duplicatas(tmp_path, template):
    fonte = FonteDeDiretorio(
        tmp_path,
        verificador=VerificadorPorTemplate(template, POSICAO_REF),
        janela_s=5.0,
    ...
    _escrever(tmp_path / "i-1", Vista.LATERAL1, _quadro())
    _escrever(tmp_path / "i-1", Vista.LATERAL2, _quadro(dx=1))
    _escrever(tmp_path / "i-1", Vista.TOPO, _quadro(dy=1))
    item = fonte.capturar("i-1", AGORA)
    assert item.duplicadas == ()
    assert len(item.vistas_utilizaveis) == 3
