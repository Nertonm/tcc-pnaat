...
OUT = ROOT / "exports/reference-envelope/r03/review03"
...
def main():
    ...
    manifest = {
        "revision": "r03", "attempt": "review03", "reference_id": None, "status": "reference_only",
        "reference_only": True,
    ...
    def historical_hashes():
        return {str(p.relative_to(ROOT)): sha(p)
                ...
                if directory.exists() for p in sorted(directory.rglob("*")) if p.is_file()} | {
                    str(p.relative_to(ROOT)): sha(p) for p in OUT.parent.rglob("*") if p.is_file() and not p.is_relative_to(OUT)}

    def statuses():
        return {name: audit_command(["git", "--no-optional-locks", "-C", str(ROOT.parent/name),
    ...
    try:
        ...
        for record, original in zip(records, components):
            ...
            # Brute-force nearest triangles plus closed-mesh solid angle containment.
            # No optional spatial index, mesh repair, or geometric input added.
            _, distances, _ = trimesh.proximity.closest_point_naive(welded, points)
            covered = np.isfinite(distances) & (distances <= 0.1)
            for index in np.flatnonzero(~covered):
                vectors = welded.triangles - points[index]
                ...
                covered[index] = np.isfinite(winding) and abs(winding) > 0.5
            stl_keepouts[record["name"]] = bool(covered.all())
        ...
        audit_command(["git", "--no-optional-locks", "status", "--short", "--untracked-files=all",
                 "--", ".", ":!latex-workspace"])
