...
OUT = ROOT / "exports/reference-envelope/r03/review02"
...
def main():
    ...
    manifest = {
        "revision": "r03", "attempt": "review02", "reference_id": None, "status": "reference_only",
        "reference_only": True,
    ...
    def historical_hashes():
        return {str(p.relative_to(ROOT)): sha(p)
                ...
                if directory.exists() for p in sorted(directory.rglob("*")) if p.is_file()} | {
                    str(p.relative_to(ROOT)): sha(p) for p in OUT.parent.iterdir() if p.is_file()}
