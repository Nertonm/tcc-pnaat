...
OUT = ROOT / 'exports/reference-envelope/ab-r01/review02'
...
def write(p, obj):
    with p.open('x') as f:
        json.dump(obj, f, indent=2, ensure_ascii=False, allow_nan=False, default=lambda v: v.item() if isinstance(v, np.generic) else str(v))
        f.write('\n')
...
def validate():
    ...
    for r in m['components']:
        ...
        s = matches[0] if matches else None  # Consume multiplicity of intentional coincident belt/forbidden solids.
        if s is not None:
            remaining.remove(s)
            serialized[r['name']] = s
        ...
        checks = dict(step_multiplicity_match=s is not None,
    ...
    for scene in ('CONVEYOR_A_REFERENCE','CONVEYOR_B_REFERENCE'):
        ...
        checks[scene+'_step_name'] = scene in step.read_text() and all(n in step.read_text() for n in required)
